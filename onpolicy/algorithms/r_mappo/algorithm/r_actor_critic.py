...
class R_Actor(nn.Module):
    def __init__(self, args, obs_space, action_space, device=torch.device("cpu")):
        ...
        if 'Dict' in obs_shape.__class__.__name__:
            ...
            self.base = MIXBase(args, obs_shape, cnn_layers_params=args.cnn_layers_params, cnn_last_linear = not (self._grid_goal or self._grid_goal_simpler))
        else:
        ...
        self.act = ACTLayer(action_space, input_size, self._use_orthogonal, self._gain, args = args, device = device)
    ...
    def dgn_analyze(self, obs):        
        ...
        attn_weight = self.base(obs, dgn_analyze=True)
        return attn_weight
...
class R_Critic(nn.Module):
    def __init__(self, args, share_obs_space, device=torch.device("cpu")):
        ...
        if self._use_popart:
            self.v_out = init_(PopArt(input_size, args.n_rollout_threads, device=device))
        else:
            self.v_out = init_(nn.Linear(input_size, 1))
