...
class SharedReplayBuffer(object):
    def __init__(self, args, num_agents, obs_space, share_obs_space, act_space):
        ...
        self.rank = np.zeros((self.episode_length + 1, self.n_rollout_threads, num_agents, 1), dtype=np.int32)
    ...
    def update_normalizer(self, value_normalizer):
        value_normalizer.update(self.returns[:-1].mean(axis=(0,2,3)))
        ...
        if self._use_proper_time_limits:
            if self._use_gae:
                ...
                for step in reversed(range(self.rewards.shape[0])):
                    if self._use_popart:
                        # step + 1
                        delta = self.rewards[step] + self.gamma * value_normalizer.denormalize(self.value_preds[step + 1], self.rank[step]) * self.masks[step + 1]  \
                            - value_normalizer.denormalize(self.value_preds[step], self.rank[step])
                        gae = delta + self.gamma * self.gae_lambda * gae * self.masks[step + 1] 
                        gae = gae * self.bad_masks[step + 1]
                        self.returns[step] = gae + value_normalizer.denormalize(self.value_preds[step], self.rank[step])
                ...
                for step in reversed(range(self.rewards.shape[0])):
                    if self._use_popart:
                        self.returns[step] = (self.returns[step + 1] * self.gamma * self.masks[step + 1] + self.rewards[step]) * self.bad_masks[step + 1] \
                                            + (1 - self.bad_masks[step + 1]) * value_normalizer.denormalize(self.value_preds[step], self.rank[step]) 
                    elif self._use_valuenorm:
                        self.returns[step] = (self.returns[step + 1] * self.gamma * self.masks[step + 1] + self.rewards[step]) * self.bad_masks[step + 1] \
    ...
    def feed_forward_generator(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        if self._mixed_obs:
            ...
            for key in self.obs.keys():
                ...
                next_obs[key] = self.obs[key][1:].reshape(-1, *self.obs[key].shape[3:])
        else:
        ...
        for indices in sampler:
            ...
            if self._mixed_obs:
                ...
                for key in obs.keys():
                    ...
                    next_obs_batch[key] = next_obs[key][indices]
            else:
            ...
            rank_batch = rank[indices]
            if advantages is None:
                adv_targ = None
            else:
                adv_targ = advantages[indices]

            yield share_obs_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch, value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, adv_targ, available_actions_batch, rank_batch, next_obs_batch
