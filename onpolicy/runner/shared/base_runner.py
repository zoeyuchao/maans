    ...
    def restore(self):
        if self.use_single_network:
            ...
            if not (self.all_args.use_eval or self.all_args.use_render):
                ...
                policy_critic_optimizer_state_dict = torch.load(str(self.model_dir) + '/critic_optimizer.pt', map_location=self.device)
                self.policy.critic_optimizer.load_state_dict(policy_critic_optimizer_state_dict)
                policy_actor_optimizer_state_dict = torch.load(str(self.model_dir) + '/actor_optimizer.pt', map_location=self.device)
                self.policy.actor_optimizer.load_state_dict(policy_actor_optimizer_state_dict)
